...
// Dynamic requires to avoid ESM loader errors in browser/sandbox environments
...
try {
	...
	// Ignore load failures (e.g. in pure Browser renderer process or strict sandbox)
}
...
export class NodeOOMDiagnosticMonitor extends CommonMonitor {
	...
	private async checkMemory(processName: string) {
		if (this.dumpLock || !this.isSampling || !v8) {
			return;
		}
