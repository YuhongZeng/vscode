...
export class ExtensionsRemoteAutoProfiler extends Disposable implements IWorkbenchContribution {
	...
	private async _onDidChangeResponsiveChange(event: IResponsiveStateChangeEvent): Promise<void> {
		...
		if (event.isResponsive && currentSession) {
			...
			let started = false;
			try {
				started = await this._remoteAgentService.startExtensionHostProfile(event.remoteConnectionToken, event.remoteProfileExtensions ?? []);
			} catch (err) {
			...
			try {
				...
				// Can throw cancellation error. That is OK, we stop profiling and analyse the profile anyways.
			}
